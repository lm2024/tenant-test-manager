...
@Slf4j
@RestController
@RequestMapping("/api/product-tenant")
@Tag(name = "产品租户管理", description = "产品租户管理相关接口")
public class ProductTenantController {
    ...
    public ResponseEntity<Map<String, Object>> healthCheck() {
        ...
        try {
            ...
            result.put("message", "产品租户管理服务健康检查完成");
            result.put("services", services);
            result.put("timestamp", LocalDateTime.now());
            
            log.info("产品租户管理服务健康检查完成");
    ...
    public ResponseEntity<Map<String, Object>> testRedisConnection() {
        ...
        try {
            if (redissonClient != null) {
                // 测试Redis连接
                String testKey = "product_tenant:test:" + System.currentTimeMillis();
    ...
    public ResponseEntity<Map<String, Object>> generateSegmentId(
            ...
            @Parameter(description = "业务类型") @RequestParam(defaultValue = "product") String bizType) {
    ...
    @Operation(summary = "产品租户信息", description = "获取产品租户的基本信息")
    @GetMapping("/info")
    @TenantSwitchHeader(headerName = "X-Tenant-ID")
    public ResponseEntity<Map<String, Object>> getProductTenantInfo(
            @Parameter(description = "租户ID") @RequestHeader(value = "X-Tenant-ID", required = false) String tenantId) {
        ...
        try {
            ...
            tenantInfo.put("tenantName", "产品租户-" + (currentTenant != null ? currentTenant : "未知"));
            tenantInfo.put("serviceName", "my-product-tenant");
            tenantInfo.put("port", 8080);
            tenantInfo.put("version", "1.0.0");
            
            result.put("status", "success");
            result.put("message", "获取产品租户信息成功");
            result.put("data", tenantInfo);
            result.put("timestamp", LocalDateTime.now());
            
            log.info("获取产品租户信息成功，租户: {}", currentTenant);
            return ResponseEntity.ok(result);
            
        } catch (Exception e) {
            result.put("status", "error");
            result.put("message", "获取产品租户信息失败: " + e.getMessage());
            result.put("timestamp", LocalDateTime.now());
            
            log.error("获取产品租户信息失败", e);
            return ResponseEntity.status(500).body(result);
        }
    }
} 